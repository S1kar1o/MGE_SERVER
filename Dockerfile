...
WORKDIR /app
EXPOSE 8080  # Render мапить внутрішній порт (10000) на 443, але вкажіть для явності
...
COPY ["MGE_HEROES.Server/MGE_HEROES.Server.csproj", "MGE_HEROES.Server/"]
RUN dotnet restore "MGE_HEROES.Server/MGE_HEROES.Server.csproj"
# Копіюємо решту файлів і публікуємо
COPY . .
WORKDIR "/src/MGE_HEROES.Server"
